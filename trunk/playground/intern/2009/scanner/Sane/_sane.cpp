...
SaneDev_set_option(SaneDevObject *self, PyObject *args)
{
  ...
  switch(d->type)
      ...
      if (!PyString_Check(value)) 
	{
      ...
      strncpy((char*)v, PyString_AsString(value), d->size-1);
      ((char*)v)[d->size-1] = 0;
      break;
    case(SANE_TYPE_BUTTON): 
    case(SANE_TYPE_GROUP):
  ...
  if (p.format == SANE_FRAME_GRAY)
    {
      switch (p.depth)
        {
          case 1:
            remain = p.bytes_per_line * im->height();
            padbytes = p.bytes_per_line - (im->width()+7)/8;
            bufpos = 0;
            lastlen = len = 0;
            while (st!=SANE_STATUS_EOF && py < im->height())
              {
                while (len > 0 && py < im->height())
                  {
                    int i, j, k;
                    j = buffer[bufpos++];
                    k = 0x80;
                    for (i = 0; i < 8 && px < im->width(); i++)
                      {
                        if(k&j)
                           im->setPixel(px++,py,qRgb(0,0,0));
                        else
                           im->setPixel(px++,py,qRgb(255,255,255));
                        k = k >> 1;
                      }
                    len--;
                    if (px >= im->width())
          ...
          case 8:
            remain = p.bytes_per_line * im->height();
            padbytes = p.bytes_per_line - im->width();
            bufpos = 0;
            len = 0;
            while (st!=SANE_STATUS_EOF && py < im->height())
              {
                while (len > 0 && py < im->height())
                  {
                    cplen = len;
                    if (px + cplen >= im->width())
                        cplen = im->width() - px;
                    for(int i=0;i<cplen;i++){
                       SANE_Byte p = buffer[bufpos++];
                       im->setPixel(px++,py,qRgb(p,p,p));
                    }
                    len -= cplen;
                    if (px >= im->width())
                      {
          ...
          case 16:
            remain = p.bytes_per_line * im->height();
            padbytes = p.bytes_per_line - 2 * im->width();
            bufpos = endian.c[0];
            lastlen = len = 0;
            while (st!=SANE_STATUS_EOF && py < im->height())
              {
                while (len > 0 && py < im->height())
                  {
                    im->setPixel(px++,py,qRgb(buffer[bufpos],buffer[bufpos],buffer[bufpos]));
                    bufpos += 2;
                    len -= 2;
                    if (px >= im->width())
                      {
      ...
      switch (p.depth)
        {
          case 1: 
            remain = p.bytes_per_line * im->height();
            padbytes = p.bytes_per_line - ((im->width()+7)/8) * 3;
            bufpos = 0;
            len = 0;
            lastlen = 0;
            SANE_Byte rgbs[8][3];
                ...
                for (color = 0; color < 3; color++)
                  {
                    ...
                    for (bit = 0; (bit < 8) && (px < pxmax); bit++)
                      {
                        rgbs[bit][color] = (val&mask) ? 0xFF : 0;
                        mask = mask >> 1;
                      }
                    if(color==2){
                      for(bit = 0; (bit < 8) && (px < im->width()); bit++){
                        im->setPixel(px++,py,qRgb(rgbs[bit][0],rgbs[bit][1],rgbs[bit][2]));
                      }
                    }
                  }
          ...
          case 16:
            if (p.depth == 8)
              {
                padbytes = p.bytes_per_line - 3 * im->width();
                bufpos = 0;
                incr = 1;
              }
            else 
              {
                padbytes = p.bytes_per_line - 6 * im->width();
                bufpos = endian.c[0];
                incr = 2;
              }
            remain = p.bytes_per_line * im->height();
                ...
                for (color = 0; color < 3; color++)
                  {
                    ...
                    if (st == SANE_STATUS_EOF) break;
                    rgb[color] = buffer[bufpos];
                    if(color == 2){
                      im->setPixel(px/3,py,qRgb(rgb[0],rgb[1],rgb[2]));
                    }
                    px++;
                    bufpos += incr;
                    len -= incr;
                    ...
                    while (len > 0)
                      {
                        ...
                        for (bit = 0; bit < 8 && px < im->width(); bit++)
                          {
                            QRgb rgb = im->pixel(px,py);
                            rgb = val&mask ? rgb | 0xFF<<offset : rgb & !(0xFF<<offset);
                            im->setPixel(px,py,rgb);
                            mask = mask >> 1;
                          }
...
PySane_get_devices(PyObject *self, PyObject *args)
{
  ...
  st=sane_get_devices((const SANE_Device***)&devlist, local_only);
  if (st) return PySane_Error(st);
  if (!(list = PyList_New(0)))
	    return NULL;
