...
class Connection(QWidget):
    ...
    def slotToggle(self, on):
        ...
        def handler(script, exception, args):
            self.view.setEnabled(True)
        comlink.link.Network.Link[self.conn.script].setState(self.conn.name, state, async=handler)
    
    def slotDelete(self):
        conn = self.conn
        m = i18n("Should I delete the\n'%s'\nconnection?")
        if KMessageBox.Yes == KMessageBox.questionYesNo(self, unicode(m) % conn.name, i18n("Delete connection?")):
            comlink.link.Network.Link[self.conn.script].deleteConnection(conn.name)
