...
MODULES_BLACKLIST = "/etc/hotplug/blacklist"
...
def editConfig(_file, add=[], remove=[]):
...
def listLoaded():
    ...
    for module, options in modules.iteritems():
        list_modules.append("%s %s" % (module, options))
    return "\n".join(list_modules)
...
def listBlacklist():
    ...
    modules = []
    if os.path.exists(MODULES_BLACKLIST):
        modules = listConfig(MODULES_BLACKLIST)
    return "\n".join(modules)

def addBlacklist(module):
    """Adds a module to blacklist."""
    editConfig(MODULES_BLACKLIST, add=[module])

def removeBlacklist(module):
    """Removes module from blacklist."""
    editConfig(MODULES_BLACKLIST, remove=[module])
